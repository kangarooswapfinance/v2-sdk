...
describe('Trade', () => {
  const token0 = new Token(ChainId.MELDCHAIN, '0x0000000000000000000000000000000000000001', 18, 't0')
  const token1 = new Token(ChainId.MELDCHAIN, '0x0000000000000000000000000000000000000002', 18, 't1')
  const token2 = new Token(ChainId.MELDCHAIN, '0x0000000000000000000000000000000000000003', 18, 't2')
  const token3 = new Token(ChainId.MELDCHAIN, '0x0000000000000000000000000000000000000004', 18, 't3')
  ...
  const pair_weth_0 = new Pair(
    new TokenAmount(WETH[ChainId.MELDCHAIN], JSBI.BigInt(1000)),
  ...
  it('can be constructed with MELDCHAIN as input', () => {
    const trade = new Trade(
      new Route([pair_weth_0], MELDCHAIN),
      CurrencyAmount.ether(JSBI.BigInt(100), MELDCHAIN),
      TradeType.EXACT_INPUT
    )
    expect(trade.inputAmount.currency).toEqual(MELDCHAIN)
    expect(trade.outputAmount.currency).toEqual(token0)
  })
  it('can be constructed with MELDCHAIN as input for exact output', () => {
    const trade = new Trade(
      new Route([pair_weth_0], MELDCHAIN, token0),
  ...
  it('can be constructed with MELDCHAIN as output', () => {
    const trade = new Trade(
      new Route([pair_weth_0], token0, MELDCHAIN),
      CurrencyAmount.ether(JSBI.BigInt(100), MELDCHAIN),
      TradeType.EXACT_OUTPUT
    )
    expect(trade.inputAmount.currency).toEqual(token0)
    expect(trade.outputAmount.currency).toEqual(MELDCHAIN)
  })
  it('can be constructed with MELDCHAIN as output for exact input', () => {
    const trade = new Trade(
      new Route([pair_weth_0], token0, MELDCHAIN),
      new TokenAmount(token0, JSBI.BigInt(100)),
      TradeType.EXACT_INPUT
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for MELDCHAIN currency input', () => {
      const result = Trade.bestTradeExactIn(
      ...
      expect(result).toHaveLength(2)
      expect(result[0].inputAmount.currency).toEqual(MELDCHAIN)
      expect(result[0].route.path).toEqual([WETH[ChainId.MELDCHAIN], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(MELDCHAIN)
      expect(result[1].route.path).toEqual([WETH[ChainId.MELDCHAIN], token0, token3])
      expect(result[1].outputAmount.currency).toEqual(token3)
    })
    it('works for MELDCHAIN currency output', () => {
      const result = Trade.bestTradeExactIn(
        [pair_weth_0, pair_0_1, pair_0_3, pair_1_3],
      ...
      expect(result[0].route.path).toEqual([token3, token0, WETH[ChainId.MELDCHAIN]])
      expect(result[0].outputAmount.currency).toEqual(MELDCHAIN)
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].route.path).toEqual([token3, token1, token0, WETH[ChainId.MELDCHAIN]])
      expect(result[1].outputAmount.currency).toEqual(MELDCHAIN)
    })
  })
